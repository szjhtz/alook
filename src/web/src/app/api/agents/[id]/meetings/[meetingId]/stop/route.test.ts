...
describe("POST .../meetings/[meetingId]/stop", () => {
  it("stops a recording meeting → completed", async () => {
    mockGet.mockResolvedValue({ id: "m1", status: MeetingStatus.RECORDING });
    mockUpdate.mockResolvedValue({ id: "m1", status: MeetingStatus.COMPLETED });
    const res = await post();
    expect(res.status).toBe(200);
    expect(mockUpdate.mock.calls[0]![3]).toMatchObject({ status: MeetingStatus.COMPLETED });
  });

  it("stops a joining meeting too", async () => {
    mockGet.mockResolvedValue({ id: "m1", status: MeetingStatus.JOINING });
    mockUpdate.mockResolvedValue({ id: "m1", status: MeetingStatus.COMPLETED });
    const res = await post();
    expect(res.status).toBe(200);
  });

  it("400 when meeting is not active (e.g. scheduled)", async () => {
    mockGet.mockResolvedValue({ id: "m1", status: MeetingStatus.SCHEDULED });
    const res = await post();
    expect(res.status).toBe(400);
    expect((await res.json()).error).toBe("meeting is not active");
  });
